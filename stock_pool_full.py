...
一天三次一条龙（早盘09:30 / 午盘12:30 / 尾盘14:20）：
  ① stock_pool.py 全量五因子生成 stock_pool.json（全量~10分钟，进度输出保活）
  ② stock_pool_ai.py 四角色AI裁决 core → watchlist.json（~5分钟，core空秒回）
  ③ short_term_ai.py 盘中分析（读刚生成的新池+刚裁决的新名单，~3分钟）
...
def run_step(name, script, env_extra=None):
    ...
    if proc.returncode != 0:
        print(f"[stock_pool_full] ❌ {name} 失败 exit={proc.returncode}", flush=True)
        sys.exit(proc.returncode)
    print(f"[stock_pool_full] ✅ {name} 完成 耗时{dur:.0f}s", flush=True)
    return dur


if __name__ == "__main__":
    t_all = time.time()
    print(f"[stock_pool_full] 🚀 股票池全流程启动 {time.strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
    # 早盘/午盘跳过监测名单清理；尾盘（14点后）才清理昨日名单
    skip_cleanup = "1" if datetime.datetime.now().hour < 14 else "0"
    run_step("股票池生成", "stock_pool.py")
    run_step("AI裁决", "stock_pool_ai.py", {"SKIP_CLEANUP": skip_cleanup})
    run_step("盘中分析", "short_term_ai.py")
    print(f"[stock_pool_full] 🎉 全流程完成 总耗时{time.time()-t_all:.0f}s", flush=True)
